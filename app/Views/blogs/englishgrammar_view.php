<?= $this->extend("layouts/base"); ?>
<?= $this->Section("content"); ?>
<!-- Blogs header starts  -->
<div id="blog-header" class="pt-2 pb-2 pt-md-5 pb-md-5">
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <img src="<?= base_url(); ?>/public/assets/images/blogs/blog-slide.png" alt="" class="img-fluid">
            </div>
        </div>
    </div>
</div>
<!-- Blogs header ends  -->
<!-- common header starts -->
<div id="common-header" class="pt-2 pb-2 pt-md-5 pb-md-5">
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <div class="text-center">
                    <h2 class= "pt-2 pb-2 text-white">Blogs</h2>
                    <div class="">
                        <nav aria-label="breadcrumb" >
                            <ol class="breadcrumb justify-content-center">
                                <li class="breadcrumb-item"><a href="<?= base_url(); ?>">Blogs</a></li>
                                <li class="breadcrumb-item active" aria-current="page">English Grammar</li>
                            </ol>
                        </nav>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<!-- common header ends -->
<!-- Welcome starts  -->
<div id="welcome" class="pt-2 pb-2 pt-md-5 pb-md-5">
    <div class="container">
        <div class="row">
            <div class="col-md-12">

                <h2 class="text-center">Welcome to A2Z ENGLISH TIPS</h2>
                <div class="row">
            <div class="col-md-4"></div>
            <div class="col-md-4">
                <h4 class="side-line text-center">
                    <span class="line"><i class="fas fa-book-reader"></i></span>
                </h4>
            </div>
            <div class="col-md-4"></div>
        </div>
                <P>Learn English Online with the free learning English videos and materials from A2Z ENGLISH TIPS. This channel will help you to learn English and improve your Pronunciation, Grammar, Spoken English, Communication Skills, Interview Skills, Life Skills, Motivations and vocabulary knowledge.</P>
                <h2>Coming soon...</h2>
            </div>
        </div>
    </div>
</div>
<!-- Welcome ends  -->

<!-- Blogs content starts  -->
<div id="blog-content" class="pt-2 pb-2 pt-md-5 pb-md-5">
    <div class="container">
        <div class="row">
            <div class="col-md-8">
                <div class="m-2">
                    <div class="row">
                        <!-- Right column starts  -->
                        <div class="col-md-4">
                            <div class="text-center">
                                <img src="<?= base_url(); ?>/public/assets/images/blogs/blog.png" alt="" class="img-fluid">
                            </div>
                        </div>
                        <div class="col-md-8">
                            <h2 class="pt-2 pb-2">Tense</h2>
                            <strong><?= date('Y-m-d') ?></strong>
                            <p>Learn English Online with the free learning English videos and materials from A2Z ENGLISH TIPS. This channel will help you to learn English and improve your Pronunciation, Grammar, Spoken English, Communication Skills, Interview Skills, Life Skills, Motivations and vocabulary knowledge.</p>
                            <a href="#">Continue Reading... </a>
                        </div>
                    </div>
                </div>
                <div class="m-2">
                    <div class="row">
                        <div class="col-md-3 col-3 p-2">
                            <ul class="list-unstyled blog-social1 text-center">
                                <li><a href="#" ><i class="fab fa-facebook m-2 fs-3"></i></a></li>
                            </ul>
                        </div>
                        <div class="col-md-3  col-3  p-2">
                            <ul class="list-unstyled blog-social2 text-center">
                                <li><a href="#" ><i class="fab fa-twitter  m-2 fs-3"></i></a></li>
                            </ul>    

                        </div>
                        <div class="col-md-3 col-3  p-2">
                            <ul class="list-unstyled blog-social3 text-center">
                                <li><a href="#" ><i class="fab fa-youtube  m-2 fs-3"></i></a></li>
                            </ul> 
                        </div>
                        <div class="col-md-3 col-3  p-2">
                            <ul class="list-unstyled blog-social4 text-center">
                                <li><a href="#" ><i class="fab fa-instagram m-2 fs-3"></i></a></li>
                            </ul> 
                        </div>
                    </div>
                </div>
                <div class="m-2">
                    <div class="row">
                        <div class="col-md-4">
                            <div class="text-center">
                                <img src="<?= base_url(); ?>/public/assets/images/blogs/blog.png" alt="" class="img-fluid">
                            </div>
                        </div>
                        <div class="col-md-8">
                            <h2 class="pt-2 pb-2">Active Voice and Passive Voice</h2>
                            <strong><?= date('Y-m-d') ?></strong>
                            <p>Learn English Online with the free learning English videos and materials from A2Z ENGLISH TIPS. This channel will help you to learn English and improve your Pronunciation, Grammar, Spoken English, Communication Skills, Interview Skills, Life Skills, Motivations and vocabulary knowledge.</p>
                            <a href="#">Continue Reading... </a>
                        </div>
                    </div>
                </div>
                <div class="m-2">
                    <div class="row">
                        <div class="col-md-3 col-3 p-2">
                            <ul class="list-unstyled blog-social1 text-center">
                                <li><a href="#" ><i class="fab fa-facebook m-2 fs-3"></i></a></li>
                            </ul>
                        </div>
                        <div class="col-md-3  col-3  p-2">
                            <ul class="list-unstyled blog-social2 text-center">
                                <li><a href="#" ><i class="fab fa-twitter  m-2 fs-3"></i></a></li>
                            </ul>    

                        </div>
                        <div class="col-md-3 col-3  p-2">
                            <ul class="list-unstyled blog-social3 text-center">
                                <li><a href="#" ><i class="fab fa-youtube  m-2 fs-3"></i></a></li>
                            </ul> 
                        </div>
                        <div class="col-md-3 col-3  p-2">
                            <ul class="list-unstyled blog-social4 text-center">
                                <li><a href="#" ><i class="fab fa-instagram m-2 fs-3"></i></a></li>
                            </ul> 
                        </div>
                    </div>
                </div>
                <div class="m-2">
                    <div class="row">
                        <div class="col-md-4">
                            <div class="text-center">
                                <img src="<?= base_url(); ?>/public/assets/images/blogs/blog.png" alt="" class="img-fluid">
                            </div>
                        </div>
                        <div class="col-md-8">
                            <h2 class="pt-2 pb-2">Direct and Indirect Speech</h2>
                            <strong><?= date('Y-m-d') ?></strong>
                            <p>Learn English Online with the free learning English videos and materials from A2Z ENGLISH TIPS. This channel will help you to learn English and improve your Pronunciation, Grammar, Spoken English, Communication Skills, Interview Skills, Life Skills, Motivations and vocabulary knowledge.</p>
                            <a href="#">Continue Reading... </a>
                        </div>
                    </div>
                </div>
                <div class="m-2">
                    <div class="row">
                        <div class="col-md-3 col-3 p-2">
                            <ul class="list-unstyled blog-social1 text-center">
                                <li><a href="#" ><i class="fab fa-facebook m-2 fs-3"></i></a></li>
                            </ul>
                        </div>
                        <div class="col-md-3  col-3  p-2">
                            <ul class="list-unstyled blog-social2 text-center">
                                <li><a href="#" ><i class="fab fa-twitter  m-2 fs-3"></i></a></li>
                            </ul>    

                        </div>
                        <div class="col-md-3 col-3  p-2">
                            <ul class="list-unstyled blog-social3 text-center">
                                <li><a href="#" ><i class="fab fa-youtube  m-2 fs-3"></i></a></li>
                            </ul> 
                        </div>
                        <div class="col-md-3 col-3  p-2">
                            <ul class="list-unstyled blog-social4 text-center">
                                <li><a href="#" ><i class="fab fa-instagram m-2 fs-3"></i></a></li>
                            </ul> 
                        </div>
                    </div>
                </div>
                <div class="m-2">
                    <div class="row">
                        <div class="col-md-4">
                            <div class="text-center">
                                <img src="<?= base_url(); ?>/public/assets/images/blogs/blog.png" alt="" class="img-fluid">
                            </div>
                        </div>
                        <div class="col-md-8">
                            <h2 class="pt-2 pb-2">Punctuations</h2>
                            <strong><?= date('Y-m-d') ?></strong>
                            <p>Learn English Online with the free learning English videos and materials from A2Z ENGLISH TIPS. This channel will help you to learn English and improve your Pronunciation, Grammar, Spoken English, Communication Skills, Interview Skills, Life Skills, Motivations and vocabulary knowledge.</p>
                            <a href="#">Continue Reading... </a>
                        </div>
                    </div>
                </div>
                <div class="m-2">
                    <div class="row">
                        <div class="col-md-3 col-3 p-2">
                            <ul class="list-unstyled blog-social1 text-center">
                                <li><a href="#" ><i class="fab fa-facebook m-2 fs-3"></i></a></li>
                            </ul>
                        </div>
                        <div class="col-md-3  col-3  p-2">
                            <ul class="list-unstyled blog-social2 text-center">
                                <li><a href="#" ><i class="fab fa-twitter  m-2 fs-3"></i></a></li>
                            </ul>    

                        </div>
                        <div class="col-md-3 col-3  p-2">
                            <ul class="list-unstyled blog-social3 text-center">
                                <li><a href="#" ><i class="fab fa-youtube  m-2 fs-3"></i></a></li>
                            </ul> 
                        </div>
                        <div class="col-md-3 col-3  p-2">
                            <ul class="list-unstyled blog-social4 text-center">
                                <li><a href="#" ><i class="fab fa-instagram m-2 fs-3"></i></a></li>
                            </ul> 
                        </div>
                    </div>
                </div>
                <div class="m-2">
                    <div class="row">
                        <div class="col-md-4">
                            <div class="text-center">
                                <img src="<?= base_url(); ?>/public/assets/images/blogs/blog.png" alt="" class="img-fluid">
                            </div>
                        </div>
                        <div class="col-md-8">
                            <h2 class="pt-2 pb-2">Transformations</h2>
                            <strong><?= date('Y-m-d') ?></strong>
                            <p>Learn English Online with the free learning English videos and materials from A2Z ENGLISH TIPS. This channel will help you to learn English and improve your Pronunciation, Grammar, Spoken English, Communication Skills, Interview Skills, Life Skills, Motivations and vocabulary knowledge.</p>
                            <a href="#">Continue Reading... </a>
                        </div>
                    </div>
                </div>
                <div class="m-2">
                    <div class="row">
                        <div class="col-md-3 col-3 p-2">
                            <ul class="list-unstyled blog-social1 text-center">
                                <li><a href="#" ><i class="fab fa-facebook m-2 fs-3"></i></a></li>
                            </ul>
                        </div>
                        <div class="col-md-3  col-3  p-2">
                            <ul class="list-unstyled blog-social2 text-center">
                                <li><a href="#" ><i class="fab fa-twitter  m-2 fs-3"></i></a></li>
                            </ul>    

                        </div>
                        <div class="col-md-3 col-3  p-2">
                            <ul class="list-unstyled blog-social3 text-center">
                                <li><a href="#" ><i class="fab fa-youtube  m-2 fs-3"></i></a></li>
                            </ul> 
                        </div>
                        <div class="col-md-3 col-3  p-2">
                            <ul class="list-unstyled blog-social4 text-center">
                                <li><a href="#" ><i class="fab fa-instagram m-2 fs-3"></i></a></li>
                            </ul> 
                        </div>
                    </div>
                </div>
            </div>
            <!-- Left column ends  -->
            <!-- Right column starts  -->
            <div class="col-md-4">
                <h4 class="side-line text-center">
                    <span class="line">Featured Video</span>
                </h4>

                <h4 class="side-line text-center">
                    <span class="line">Popular Posts</span>

                </h4>
                <div class="pt-5 pb-5">
                    <div class="m-2">
                        <div class="row">
                            <div class="col-md-4 col-4">
                                <div class="text-center">
                                    <img src="<?= base_url(); ?>/public/assets/images/blogs/blog.png" alt="" class="img-fluid">
                                </div>
                            </div>
                            <div class="col-md-8 col-8">
                                <h5 class="">Introduction</h5>
                                <a href="#">Click Here</a>
                                <p>4K Total Views</p>
                            </div>
                        </div>
                    </div>
                    <div class="m-2">
                        <div class="row">
                            <div class="col-md-4 col-4">
                                <div class="text-center">
                                    <img src="<?= base_url(); ?>/public/assets/images/blogs/blog.png" alt="" class="img-fluid">
                                </div>
                            </div>
                            <div class="col-md-8 col-8">
                                <h5 class="">Introduction</h5>
                                <a href="#">Click Here</a>
                                <p>4K Total Views</p>
                            </div>
                        </div>
                    </div>
                    <div class="m-2">
                        <div class="row">
                            <div class="col-md-4 col-4">
                                <div class="text-center">
                                    <img src="<?= base_url(); ?>/public/assets/images/blogs/blog.png" alt="" class="img-fluid">
                                </div>
                            </div>
                            <div class="col-md-8 col-8">
                                <h5 class="">Introduction</h5>
                                <a href="#">Click Here</a>
                                <p>4K Total Views</p>
                            </div>
                        </div>
                    </div>
                    <div class="m-2">
                        <div class="row">
                            <div class="col-md-4 col-4">
                                <div class="text-center">
                                    <img src="<?= base_url(); ?>/public/assets/images/blogs/blog.png" alt="" class="img-fluid">
                                </div>
                            </div>
                            <div class="col-md-8 col-8">
                                <h5 class="">Introduction</h5>
                                <a href="#">Click Here</a>
                                <p>4K Total Views</p>
                            </div>
                        </div>
                    </div>
                    <div class="m-2">
                        <div class="row">
                            <div class="col-md-4 col-4">
                                <div class="text-center">
                                    <img src="<?= base_url(); ?>/public/assets/images/blogs/blog.png" alt="" class="img-fluid">
                                </div>
                            </div>
                            <div class="col-md-8 col-8">
                                <h5 class="">Introduction</h5>
                                <a href="#">Click Here</a>
                                <p>4K Total Views</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="pt-2 pb-2">
                    <h4 class="side-line text-center">
                        <span class="line">Search</span>

                    </h4>
                    <form class="d-flex">
                        <input class="form-control me-2" type="search" placeholder="Search" aria-label="Search">
                        <button class="btn btn-outline-success" type="submit">Search</button>
                    </form>
                </div>
                <div class="pt-3 pb-3">
                    <h4 class="side-line text-center">
                        <span class="line">Categories</span>

                    </h4>
                    <select class="form-select" aria-label="Default select example">
                        <option selected>Open this select menu</option>
                        <option value="1">One</option>
                        <option value="2">Two</option>
                        <option value="3">Three</option>
                    </select>
                </div>
                <div class="pt-3 pb-3">
                    <h4 class="side-line text-center">
                        <span class="line">Archieves</span>

                    </h4>
                    <ul class="list-unstyled">
                        <li><a href="#">September 2021</a></li>
                        <li><a href="#">August 2021</a></li>
                        <li><a href="#">July 2021</a></li>
                        <li><a href="#">June 2021</a></li>
                        <li><a href="#">May 2021</a></li>
                        <li><a href="#">April 2021</a></li>
                        <li><a href="#">March 2021</a></li>
                        <li><a href="#">Febraury 2021</a></li>
                        <li><a href="#">January 2021</a></li>
                    </ul>

                </div>
                <!-- Right column ends  -->
            </div>
        </div>
    </div>
</div>
<!-- Blogs content ends  -->

<?= $this->endSection(); ?>